...
test.describe("界面冒烟", () => {
  test("页面加载：标题与四个页签就位", async ({ page }) => {
    await page.goto("/");
    await expect(page.getByRole("heading", { name: "Dataset Factory 打标测试台" })).toBeVisible();
...
test.describe("打标全链路", () => {
  test("发指令打标：假模型回复直达界面（浏览器 → HTTP → 引擎 → 假端点）", async ({ page }) => {
    ...
    // 聊天 tab：选提示词、发指令、等待假模型回复上屏。
    await page.getByRole("button", { name: "聊天打标" }).click();
